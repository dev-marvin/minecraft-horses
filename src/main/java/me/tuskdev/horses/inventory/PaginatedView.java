...
import static me.tuskdev.horses.inventory.PaginatedViewContext.FIRST_PAGE;
...
public abstract class PaginatedView<T> extends View {
    ...
    public PaginatedViewSlide<T> getPaginatedViewSlide() {
        return paginatedViewSlide;
    }

    void setPaginatedViewSlide(PaginatedViewSlide<T> paginatedViewSlide) {
	...
	final void updateContext(PaginatedViewContext<T> context, int page, boolean pageChecking, boolean render) {
		...
		if (pageChecking) {
			// index check
			if (render && !context.getPaginatedViewSlide().hasPage(page)) {
				getFrame().debug("[context] paginated update - no page " + page + " available");
				getFrame().debug("[context] paginated update - items: " + context.getPaginatedViewSlide().getSource().stream().map(Object::toString).collect(Collectors.joining(", ")));
				getFrame().debug("[context] paginated update - count: " + context.getPaginatedViewSlide().count());
				getFrame().debug("[context] paginated update - page size: " + context.getPaginatedViewSlide().getPageSize());
				return;
			}
    ...
    private void renderLayout(PaginatedViewContext<T> context, String[] layout, ViewItem[] preservedItems) {
        ...
        final List<T> elements = context.getPaginatedViewSlide().getPage(context.getPage());
    ...
    void resolveLayout(PaginatedViewContext<T> context, String[] layout, boolean render) {
		...
		context.getPaginatedViewSlide().setPageSize(context.itemsLayer.size());
    ...
    final ViewItem[] clearLayout(PaginatedViewContext<T> context, String[] layout) {
        ...
        final int size = context.getPaginatedViewSlide().getPage(context.getPage()).size();
        final int layerSize = layout == null ? 0 /* ignored */ : context.getItemsLayer().size();
